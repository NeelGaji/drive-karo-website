    ...
    const mockSessions: ActiveSession[] = [
      {
        ...
        startTime: '10:00 AM',
        currentPosition: { lat: 21.1959, lng: 72.8302, timestamp: Date.now() },
        routePath: [
          { lat: 21.1959, lng: 72.8302, timestamp: Date.now() - 1800000 },
          { lat: 21.1969, lng: 72.8312, timestamp: Date.now() - 1500000 },
          { lat: 21.1979, lng: 72.8322, timestamp: Date.now() - 1200000 },
          { lat: 21.1989, lng: 72.8332, timestamp: Date.now() - 900000 },
          { lat: 21.1999, lng: 72.8342, timestamp: Date.now() - 600000 },
          { lat: 21.2009, lng: 72.8352, timestamp: Date.now() - 300000 },
          { lat: 21.1959, lng: 72.8302, timestamp: Date.now() },
        ],
        distanceTraveled: 0,
      ...
      },
      {
        ...
        startTime: '2:00 PM',
        currentPosition: { lat: 21.2059, lng: 72.8402, timestamp: Date.now() },
        routePath: [
          { lat: 21.2059, lng: 72.8402, timestamp: Date.now() - 1200000 },
          { lat: 21.2069, lng: 72.8412, timestamp: Date.now() - 900000 },
          { lat: 21.2079, lng: 72.8422, timestamp: Date.now() - 600000 },
          { lat: 21.2089, lng: 72.8432, timestamp: Date.now() - 300000 },
          { lat: 21.2059, lng: 72.8402, timestamp: Date.now() },
        ],
        distanceTraveled: 0,
        sessionStatus: 'active',
            ...
            <div key={session.id} className="bg-[#434546] rounded-lg overflow-hidden">
              ...
              <div className="h-48 relative cursor-pointer" onClick={() => openFullscreen(session)}>
                ...
                >
                  ...
                  {/* Route Path */}
                  <Polyline
                    positions={session.routePath.map(point => new LatLng(point.lat, point.lng))}
                    color="#FFD43B"
                    weight={3}
                    opacity={0.8}
                  />
                  ...
                  <Marker position={new LatLng(session.currentPosition.lat, session.currentPosition.lng)}>
                    <Popup>
          ...
          <div className="bg-[#8B8E8F] rounded-xl w-full h-full max-w-6xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 relative z-0">
              ...
              >
                ...
                {/* Route Path */}
                <Polyline
                  positions={selectedSession.routePath.map(point => new LatLng(point.lat, point.lng))}
                  color="#FFD43B"
                  weight={4}
                  opacity={0.8}
                />
                ...
                {selectedSession.routePath.length > 0 && (
                  <Marker position={new LatLng(selectedSession.routePath[0].lat, selectedSession.routePath[0].lng)}>
                    <Popup>
                      <div className="text-sm">
                ...
                {/* Current Position Marker */}
                <Marker position={new LatLng(selectedSession.currentPosition.lat, selectedSession.currentPosition.lng)}>
                  <Popup>
                    <div className="text-sm">
